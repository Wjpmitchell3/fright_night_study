---
title: "ER Choice v. Usage - Prelim Study Analysis"
author: "W.J. Mitchell"
date: "`r Sys.Date()`"
...
```{r Setting Working Directories}
  ...
  Export <- paste0(WorkDir, "/Data/Prelim_Study/Deriv/")
  Import <- paste0(WorkDir, "/Data/Prelim_Study/Raw/")
...
```{r Loading dataframe}
  ...
  df_all <- read.csv(file = paste0(Import, "df.csv"), 
                 header = T, sep=",", 
                 stringsAsFactors = F,
                 na.strings=c("","N/A"), 
                 row.names = 1)
...
```{r Removing Intensities Lacking Endorsement}
  ...
  df_all$Emo.Extent[is.na(df_all$Presence)] <- NA
  df_all$Reg.Extent[is.na(df_all$Presence)] <- NA
  df_all$Reg.Extent[df_all$Reg.Extent == 1] <- NA
```  
...
```{r Building an Empty Dataframe to Calculate Chronbachs Alpha}
  ...
  alpha <- data.frame(PID = sort(rep(unique(df_all$PID), 10)), 
                      Event = rep(1:10,47), 
                      Alert = NA,
                      Amuse = NA,
                      Excite = NA,
...
for (row in 1:nrow(df_all)){
  if (row %% 14 != 0) {
    if (!is.na(df_all$Presence[row])) {
      if (df_all$Presence[row] == 1) {
        alpha[ceiling(row/14), (2 + row %% 14)] <- df_all$Emo.Extent[row]
      }
    }
  }
}
...
Much to my chagrin, the existing event variable notes the order in which events were recalled, not the chronological order in which the events occurred. Those are captured in the "EventsOrdered" columns. It's confusing to wrap your head around but the number that exists in a cell in those columns represents which event, in recall order, occurred first chronologically. So if df_all$EventsOrdered[1] read "7", then that would mean that the 7th event that that participant recalled occurred first chronologically. We care about both, so I'm going to create an order column for both.

```{r Creating a variable tracking the order in which events were entered}
# Creating an empty variable in the dataframe
df_all$Event_chronolorder <- NA

# Iterating through each row ...
for (row in 1:nrow(df_all)){
  # and each column of events ...
  for (col in grep(x = names(df_all),
                 pattern = "EventsOrdered\\.*")){
    # and recording the appropriate index for each event.
    if (df_all$Event[row] == df_all[row,col]){
      df_all$Event_chronolorder[row] <- col + 1 - grep(x = names(df_all),
...
```{r Making the intercept of event the first event}
  df_all$Event_recallorder <- as.numeric(df_all$Event - 4.5)
  df_all$Event_chronolorder <- as.numeric(df_all$Event_chronolorder - 3.5)
```
...
```{r Subsetting Relevant Data}
  df_all <- subset(df_all,
               !is.na(df_all$Emo.Extent),
               select = names(df_all)[c(1:2, 202:203, 6:11, 14, 16, 34:35, 38, 46,
...
df_all <- dplyr::group_by(df_all, as.factor(PID))
...
df_all <- group_by(df_all, PID)
df_all$Emo.Extent.pmean.z <- as.numeric(scale(df_all$Emo.Extent.pmean, scale = T, center = T))
df_all$Reg.Extent.pmean.z <- as.numeric(scale(df_all$Reg.Extent.pmean, scale = T, center = T))
df_all <- ungroup(df_all)
```

```{r Z Scoring Variables}
  df_all$Emo.Extent.z <- as.numeric(scale(df_all$Emo.Extent, scale=T, center=T))
  df_all$Reg.Extent.z <- as.numeric(scale(df_all$Reg.Extent, scale=T, center=T))
  df_all$Emo.Extent.pc.z <- as.numeric(scale(df_all$Emo.Extent.pc, scale=T, center=T))
  df_all$Reg.Extent.pc.z <- as.numeric(scale(df_all$Reg.Extent.pc, scale=T, center=T))
```
...
# Coercing the easy variables
  df_all$Emotion <- as.factor(df_all$Emotion)
  df_all$Valence <- as.factor(df_all$Valence)
...
# fix those as well (e.g., 3-5 becomes "5-March")
  df_all$HauntedHouse_Times[df_all$HauntedHouse_Times == "."] <- NA
  df_all$HauntedHouse_Times[df_all$HauntedHouse_Times == "3-Feb"] <- 2.5
  df_all$HauntedHouse_Times[df_all$HauntedHouse_Times == "4-5 times"] <- 4.5
  df_all$HauntedHouse_Times[df_all$HauntedHouse_Times == "5-Mar"] <- 4
  df_all$HauntedHouse_Times[df_all$HauntedHouse_Times == "2 or 3 (ten + years ago)"] <- 2.5
  df_all$HauntedHouse_Times[df_all$HauntedHouse_Times == "Once every couple of years"] <- NA
  df_all$HauntedHouse_Compare[df_all$HauntedHouse_Compare == "."] <- NA
  
# Now we'll coerce the other ones. 
  df_all$HauntedHouse_Times <- as.numeric(df_all$HauntedHouse_Times)
  df_all$HauntedHouse_Compare <- as.numeric(df_all$HauntedHouse_Compare)
```

```{r Adding Regulated Factor}  
  df_all$Regulated <- NA
  df_all$Regulated[df_all$Reg.Extent > 1] <- "Regulated"
  df_all$Regulated[df_all$Reg.Extent < 2 | is.na(df_all$Reg.Extent)] <- "Not Regulated"
  df_all$Regulated <- as.factor(df_all$Regulated)
```
...
```{r Primary Dataframe}
df <- subset(df_all, df_all$Regulated == "Regulated" & df_all$Valence == "Negative" & df_all$MultipleStrategies == 0 & 
               ((df_all$Reappraisal == 1 & df_all$Attention.Deployment == 0) | (df_all$Reappraisal == 0 & df_all$Attention.Deployment == 1)))
...
cols_old <- c("Event.ID", "Event_chronolorder", "Attention.Deployment", "Emotion", "Emo.Extent", "Emo.Extent.z", "Emo.Extent.pc.z",
              ...
              "Reg.Extent.pmean.z", "PID", "ERQ_CogReapp", "ERQ_ExpSuppress", "IUS", "IUS_Factor1", "IUS_Factor2", "BDI")
...
cols_new <- c("Event", "Event_chronolorder", "Strategy.OG", "Emotion", "Emo.Extent", "Emo.Extent.z", "Emo.Extent.pc.z",
              "Reg.Extent", "Reg.Extent.z", "Reg.Extent.pc.z", "Emo.Extent.pmean.z",
              "Reg.Extent.pmean.z", "PID.OG", "ERQ_Reapp.OG", "ERQ_Supp.OG", "IUS.OG", "IUS_F1.OG", "IUS_F2.OG", 
              "BDI")
...
```{r Exporting Regulation & Intensity Regression}
  tiff(paste0(Plots, "Archive/Fig2.tiff"),       
       res = 300,
       units = "in",
       width = 12, 
       height = 9)
...
This was actually conducted Post-Hoc (i.e., not preregistered), so let me just be clear right out of the gate. Someone has asked me the extent to which anxiety might be a motivating factor to regulate and it got me thinking that using a validated measure of state anxiety may be a means of assessing the motivation to self-regulate. 

```{r Creating a person-level dataframe}
df_pl <- df_all %>%
         subset(!is.na(.$Reg.Extent),
                select = c("PID", "STAI_State", "Reg.Extent", "Event.ID")) %>%
         group_by(PID) %>%
         summarise(STAI_State = mean(STAI_State, na.rm = TRUE),
                   Avg_Reg_Extent = mean(Reg.Extent, na.rm = TRUE)
         )
```

```{r Rescaling variables}
df_pl$STAI_State_z <- scale(df_pl$STAI_State) %>%
                    as.numeric()
df_pl$Avg_Reg_Extent_z <- scale(df_pl$Avg_Reg_Extent) %>%
                    as.numeric()
```

```{r Running analyses}
m1 <- lm(Avg_Reg_Extent_z ~ STAI_State_z, data = df_pl,)
summary(m1)
confint(m1, level = 0.95)
rm(m1,df_pl)
```

# Yet Another Preliminary Analysis
...
```{r Exporting Regulation & Intensity Regression}
  tiff(paste0(Plots,"Archive/Fig3.tiff"),
       res = 300,
       units = "in",
       width = 12, 
       height = 9)
...
for (i in 1:nrow(df_temp)){
  ...
  df_temp$IUS[i] <- df$IUS[df$PID == df_temp$PID[i] &
                                 df$Event_chronolorder == df_temp$Event_chronolorder[i]][1]  
  df_temp$IUS_Factor1[i] <- df$IUS_Factor1[df$PID == df_temp$PID[i] &
                               df$Event_chronolorder == df_temp$Event_chronolorder[i]][1] 

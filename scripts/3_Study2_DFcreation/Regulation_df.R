...
# It may be the case that some participants do not have data at each of 
# the possible points: Pre-Exposure, Post-Exposure, and Delay. Here, I'm 
# checking how many entries each participant has provided and telling R 
# to identify which participants don't have three entries.

## Checking Number of Entries ----
which(table(df$PID) != 3)

# In reality, no regulation occurred prior to exposure, so I'm really only 
# interested in datapoints after exposure. I'd hate to exclude someone from 
# analysis just because they are missing data from a point that's irrelevant 
# to these specific analyses. Therefore, I'm going to remove that pre-exposure 
# data from this dataframe and check now that each participant has two entries.

## Removing Pre-Exposure ----
df <- subset(df, df$Stage != "Pre-Exposure")
(issues <- sort(unique(df$PID))[which(table(df$PID) != 2)])

# We still have a few participants with more or less than two datapoints. Many 
# of the participants with more than two entries are due to errors where we had 
# to restart the qualtrics survey, so those can be removed by identifying the 
# incomplete entries for each participant that was flagged as an issue.
...
# Now let's run the check again and see how many participants still do not have 2 
# points of data.
...
# Not all remaining variables are relevant to regulation. Some are physiology. 
# Some are memory. Either way, I'm going to isolate those variables we want. 
# Unfortunately, because the original naming conventions were so poor, I have no 
# real choice other than to specify by index which columns I need, which is pretty 
# susceptible to human error, but we just have to be careful. 

## Isolating Relevant variables ----  
df_reg <- df[,c(18:19, 878:956, 713:804, 957:1122)]

# Even within the big chunks we cut out, there are superfluous variables. The timing 
# columns are not relevant for these analyses. Entry was too inconsistent to reliably 
# use as a measure due to the conditions in which this data was collected. Therefore, 
# I'm removing them.
...
# We want to know what the hell we're looking at and these current column names are 
# going to be a major problem. Luckily, the order of the data follows a predictable 
# pattern, so we're going to write new column names so that we can be more exact in 
# our variable calls and such. 
...
# Here I'm creating two arrays. Which one is used depends upon what stage the data was 
# asked at. At the Immediate post-exposure stage, we specified a section from which we 
# expected people to recall events. With the Delayed post-exposure, we left it open. 
...
# and then assembling all of those into a new concatenated array, which we 
# then apply to the column names of our new regulation dataframe. Then I'm 
# going to sample the first 20 column names to make sure I did it right.
...
# Checking whether there are any rows that have data where it shouldn't be, 
# based upon other data present (e.g., if someone reports regulating an emotion, 
# but doesn't report experiencing an emotion)

...
rm(i,j,df_compare,Events_recalled,df_imm, df_mem, tracker, Events_total)
```
...
plot <- ggplot(m1.e, aes(x=Intense.z, y=fit)) +
  geom_jitter(data = df, aes(x=Intense.z, y=Distracted, color = Distracted), alpha = 0.5, width = 0.5, height = 0.01) +
...
```{r Exporting Regulation & Intensity Regression}
  setwd(dir_plots)
  pdf("FN03_EmoExtent_Strategy.pdf",
      width = 4, 
      height = 3)
  plot
  dev.off()
...
df$fit <- fitted(m3)
plot <- ggplot(data = df, aes(x = Intense.z, y = fit, group = Distracted_cat, color = Distracted_cat)) +
                stat_smooth(aes(x = Intense.z, y = RegSuccess.z, color = as.factor(PID)),
                            method = "lm",
                            alpha = .00,
                            size = .5) +
                stat_smooth(method="lm", se = T, alpha = .6, size = 1.5) +
                scale_x_continuous(breaks = c(-3,-2,-1,0,1,2,3)) +
                scale_y_continuous(breaks = c(-2,-1,0,1,2)) +
                scale_color_nord(name = "Strategy Usage",
                                 labels = c("Distraction", "Reappraisal"),
                                 palette = "afternoon_prarie")+
                labs(title = "Emotion Intensity & Strategy Predicts Regulation Success",
                     subtitle = "Interaction Beta = 0.253 **",
                     x = "Emotion Intensity (z)", 
...
```{r Exporting Regulation & Intensity Regression}
  setwd(dir_plots)
  pdf("FN03_EmoExtent_RegSuccess.pdf",
      width = 4, 
      height = 3)
  plot
  dev.off()
...
If contextualization is the mechanism, than individuals with more internal memories in their recall might be more similar to lab studies

### Data Creation

```{r Creating an AI Dataframe}
df_ai <- data.frame("PID" = sort(rep(unique(df_all$PID[!is.na(df_all$PID)]), 4)),
                    "Timing" = rep(x = c("immediate", "delay"),
                                   times = length(unique(df_all$PID[!is.na(df_all$PID)])) * 2),
                    "Section" = rep(x = sort(rep(x = c("crypt", "machine shop"), 
                                                 times = 2)), 
                                    times = length(unique(df_all$PID[!is.na(df_all$PID)]))),
                    "AI_InternalSum" = NA,
                    "AI_ExternalSum" = NA,
                    "AI_PropIntExt" = NA)
```

```{r Loading in the AI data}
for (i in 1:nrow(df_ai)){
  for (j in unique(df_ai$Section)){
    for (k in unique(df_ai$Timing)){
      if (j == "crypt" & k == "immediate"){
        abbrev <- paste0(k, "/",j,"/I_C_")
      }
      if (j == "machine shop" & k == "immediate"){
        abbrev <- paste0(k, "/",j,"/I_MS_")
      }
      if (j == "crypt" & k == "delay"){
        abbrev <- paste0(k, "/",j,"/D_C_")
      }
      if (j == "machine shop" & k == "delay"){
        abbrev <- paste0(k, "/",j,"/D_MS_")
      }
      path <- paste0("S:/Murty_Group/studies/frightnight.02/data/raw/AI Transcripts 06.07.2022/",
                      abbrev, df_ai$PID[i], ".csv")
      if (file.exists(path)){
        df_temp <- read.csv(file = path,
                            header = F,
                            na.strings = c("", "NA", "na"))[,1:2]
        if (nrow(df_temp) > 4){
          names(df_temp) <- df_temp[4,]
          df_temp <- df_temp[-c(1:4),]
          df_temp$Internal <- 0
          df_temp$Internal[df_temp$AI < 6] <- 1
          df_ai$AI_InternalSum[df_ai$PID == df_ai$PID[i] & 
                               df_ai$Timing == k & 
                               df_ai$Section == j] <- sum(df_temp$Internal)
          df_ai$AI_ExternalSum[df_ai$PID == df_ai$PID[i] & 
                               df_ai$Timing == k & 
                               df_ai$Section == j] <- nrow(df_temp) - sum(df_temp$Internal)
...
df_ai$AI_PropIntExt <- df_ai$AI_InternalSum / (df_ai$AI_InternalSum + df_ai$AI_ExternalSum) 
```

```{r Move that data to the primary dataframe}
df_tl <- rbind(df_pl, df_pl)
df_tl$Timing <- sort(rep(c("immediate", "delay"), nrow(df_pl)))
df_tl$AI <- NA
df_tl$PropDist <- NA
for (i in unique(df_pl$PID)){
  for (j in unique(df_tl$Timing)){
    df_tl$AI[df_tl$PID == i & df_tl$Timing == j] <- mean(df_ai$AI_InternalSum[df_ai$PID == i & df_ai$Timing == j], na.rm = T)
  }
}

df_tl$Stage <- NA
df_tl$Stage[df_tl$Timing == "delay"] <- "One Week Post-Exposure"
df_tl$Stage[df_tl$Timing == "immediate"] <- "Immediate Post-Exposure"
for (i in unique(df_tl$PID)){
  for (j in unique(df_tl$Stage)){
     total <- nrow(subset(df, df$PID == i & df$Stage == j))
     dist <- nrow(subset(df, df$PID == i & df$Stage == j & df$Distracted == 1))
     if (total > 0) {
        df_tl$PropDist[df_tl$PID == i & df_tl$Stage == j] <- dist/total
     } 
  }
}
...
```{r AI Models}
m0 <- lmer(PropDist ~ 1 + (1 | PID), data = subset(df_tl, !is.na(df_tl$AI)), REML = F)
m1 <- lmer(PropDist ~ AI + (1 | PID), data = subset(df_tl, !is.na(df_tl$AI)), REML = F)
m2 <- lmer(PropDist ~ AI + Intense.pmean.z + (1 | PID), data = subset(df_tl, !is.na(df_tl$AI)), REML = F)
m3 <- lmer(PropDist ~ AI + Intense.pmean.z + Stage + (1 | PID), data = subset(df_tl, !is.na(df_tl$AI)), REML = F)
```
...
stargazer(m0,m1,m2,m3,
          type = "text", 
          title= "AI Predicting Regulation Usage", 
          column.labels = c("Null Model", "Model 1", "Model 2", "Model 3"),
          dep.var.labels = "Strategy Usage", 
          star.cutoffs = c(0.05, 0.01,0.001),
          covariate.labels = c("Num. of Internal Details", "Intensity (z)", "Stage, Delay", "Intercept"),
          ci = T,
          notes = "")
```